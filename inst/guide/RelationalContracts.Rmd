...
vec.trans.fun = function(ax.df,x.max,...) {
...
g = rel_game("Slowly Intensifying Repeated Principal-Agent") %>%
  ...
  rel_states(x.df,
    A.fun = A.fun,
    pi1 = ~ ifelse(state=="m",0, ifelse(b1=="b" | b2=="b", -x1, e)),
    pi2 = ~ ifelse(state=="m",0, ifelse(b1=="b" | b2=="b", -x2, - 1/2 * e^2)),
