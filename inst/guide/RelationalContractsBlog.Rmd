...
One goal of game theory is to understand how effective cooperation can be sustained in relationships. This blog post illustrates the new R package `RelationalContracts` that facilitates game theoretic analysis of long-run relationships. I also want to introduce you the problem of equilibrium selection, the vulnerability paradox and how one can explicitly account for repeated negotiations in relationships. 
The most well known game is probably the prisoners' dilemma. 
...
g = rel_game("Mutual Gift Game") %>%
  rel_state("x0",
    ...
    A1 = list(e1=seq(0,1,by=0.2)),
    A2 = list(e2=seq(0,1,by=0.2)),
  ...
  rel_compile()
```
Each player $i$ chooses an effort level $e_i$ on a fine grid between 0 and 1 that directly benefits the other player. Effort involves for player $i$ cost of $\frac 1 2 e_i$ and grants the other player a benefit of $e_i$. While it would be jointly efficient if both players choose full effort $e_1=e_2=1$, the unique Nash equilibrium of the one-shot game is that both players choose zero effort.

Yet, if players interact repeatedly, positive effort levels can be sustained in subgame perfect equilibria. The common assumption in the relational contracting literature is that players interact for infinitely many periods and discount future payoffs with a discount factor $\delta \in [0,1)$. Discounting can be due to positive interest rates, or one can alternatively interpret $\delta$ as 1 minus an exogenous probability that the relationship breaks down.
...
In our example game so called *grim-trigger* strategies implement the harshest possible punishment: should any player ever deviate from the agreed upon effort level then in all future periods players will play the Nash equilibrium of the one shot game, i.e. they both chose zero effort.

One can easily show that such grim-trigger strategies can implement a symetric effort level $e$ on the equilibrium path if and only if $$e - 0.5 \cdot e^2 \geq (1-\delta) e.$$ The LHS is the average discounted payoff of a player on the equilibrium path. Average discounted payoffs are just the discounted sum of payoffs multiplied by $(1-\delta)$. This normalization puts payoffs of the repeated game on the same scale than payoffs of the one shot game. The RHS is the average discounted continuation payoff if a player deviates to zero effort. In the period of deviation she saves her effort cost but in all future periods payoffs are zero.

However, there are many more strategies that constitute a subgame perfect equilibrium (SPE). For example, another SPE of the repeated game is to always repeat the stage game Nash equilibrium of choosing zero effort. In other SPE, players may pick positive but lower effort levels than $0.6$. Effort levels may also be asymmetric or vary over time. Also different punishment schemes can be used in n equilibrium, e.g. punishments that reduce effort only for a finite number of periods or asymmetric punishments that are more costly for the deviator than the other player.

For sufficiently large discount factor there are infinitely many different equilibria and infinitely many different equilibrium payoffs even if one restricts attention to pure strategies. 

To compute the set of SPE payoffs in a tractable fashion, one typically imposes additional structure on the game. One assumption is that at the beginning of a period all players observe the realization of standard uniform random variable that can be used as a public correlation device.
...
rsg.sol = solveSG(rsg=rsg,delta=0.3,verbose = 0L)
...
```
The library [RSGSolve](https://github.com/skranz/RSGSolve) is just a simple R interface to Benjamin Brook's package [SGsolve](https://github.com/babrooks/SGSolve) (developed with Dilip Abreu and Yuli Sannikov) that implements [their 2016 algorithm](http://www.benjaminbrooks.net/downloads/abs_stochasticgames.pdf).

Many things become easier and much faster to compute (relevant for larger games) if one makes the additional assumption that at the beginning of each period, before actions take place, players can make voluntary monetary transfers to each other (or to an uninvolved third party) and that players are risk-neutral. This a common assumption in the relational contracting literature. Susanne Goldlücke and me have developed methods ( [2012](http://www.wiwi.uni-bonn.de/kranz/Infinitely%20Repeated%20Games%20with%20Public%20Monitoring%20and%20Monetary%20Transfers.pdf) and [2017](https://www.uni-ulm.de/fileadmin/website_uni_ulm/mawi.inst.160/pdf_dokumente/mitarbeiter/kranz/dyngame.pdf)) to compute and characterize equilibrium payoff sets in this class of games.
