...
## Specifying a simple repeated game
...
The parameters `pi1` and `pi2` specify payoffs for both players. Payoffs will usually be formula's (starting with `~`), which means that they can depend on the chosen action or parameters specified for the game or state.

## A game with 2 states and transitions
...
## Specifying Multiple States at Once
...
The function `rel_transition` allows to specify multiple source states `xs` and multiple destination states `xd`. Here choosing `inc_vul` increases the state number as long as we have not reached yet state `x=5`.
...
One way to specify different action spaces for different subsets of states is to add multiple `rel_states` command, as in the example below:
```{r}
effort.seq = seq(0,1,by=0.1)
g = rel_game() %>%
  rel_states(x = 0:4,
...
## Specify action spaces via A.fun instead of A1 and A2 
...
## Specify payoffs via pi.fun instead of pi1 and pi2

The argument `pi.fun` of `rel_states` allows to specify payoffs via a vectorized functions instead of providing formulas to the arguments `pi1` and `pi2`. Here is an example:

```{r}
effort.seq = seq(0,1,by=0.1)
pi.fun = function(ax.df, ...) {
  restore.point("pi.fun") # for debugging purposes
  mutate(ax.df,
    pi1 = ifelse(e2 != -1,e2, -x/5) - 0.5*e1^2,
    pi2 = e1 - 0.5*pmax(e2,0)^2
  )
...
The column `x` specifies a state, the column `.a` will probably not be used, it is just a unique index of the action profile number in the states. The other columns correspond to the action names. (If some action is not present in some state, it is filled with `NA`)
...
## Specify state transitions via trans.fun

As an alternative to specifying state transitions with one or several calls to `rel_transition`, one can use the argument `trans.fun` or `rel_states`. Here is an example:
```{r}
...
`trans.fun` is a function that takes the same arguments as explained in the subsection about `pi.fun` above. It returns a data frame that whose columns are essentially the same as the arguments of `rel_transition`. Each row specifies a transition from the state in column `xs` to the state in column `xd` with probability of column `prob` given that the actions take the values specified in additional columns. 
...
## State Space with multiple dimensions and helper variables: A Cournot Duopoly with Capacity Investments

The following examples will be variants of a dynamic Cournot duopoly with capacity investments. A state `x` is characterized by a tuple `(x1,x2)` that describes the production capacities of each player. In each period players simultaneously choose integer outputs `q1` and `q2` between 0 and their capacity and decide whether to invest into new capacity, at least as long as the maximum capacity `x.max` has not been reached.
...
First note that in the call to `rel_states`, the argument `x` is not a vector of state names, but instead a data frame specified further above. The data frame has a column `x` but also two additional columns `x1` and `x2` that provide additional information about each state, namely the capacities of each player. The column `x` that uniquely identifies each state is just a label pasting `x1` and `x2` together.

The additional state variables `x1` and `x2`could be used in the formula's for `pi1` and `pi2`, are passed as additional arguments to `A.fun`, and are additional columns in the `ax.df` argument passed to `trans.fun`. You can add as many columns to such a state data frame as you like. The only requirement is that there is a column `x` with unique values in each row. 
...
```
Relevant to compute state transitions are only the investment actions and the state variables. We thus select the relevant columns and reduce to unique rows and get:
...
## Specifying state transitions using the helper function `independent_transitions`: Cournot duopoly where investments are not always successful
...
```
Inside the call to `irv_joint_dist` we specify with `irv_var` two discrete valued random variables `new_x1` and `new_x2` that shall be independently distributed from each other.
Inside `irv_var` we call `irv_val` to specify values that the variable can take and their realiziation probabilities. These values and probabilities are computed using the columns of the data frame `df` passed to `irv_joint_dist` using lazy evaluation.

For convenience, we can also set a default value for each random variables that is realized with probability 1 minus the some of all explicitly specified probabilities via `irv_val`. E.g. without default values, we could have equivalently specified `new_x1` by
```{r eval=FALSE}
  irv("new_x1",
    irv_val(val=x1+1,prob=i1*sp),
...
## Advanced use of `irv_joint_dist`: Cournot duopoly where investments are not always successful and capacity may depreciate
...
sp = 0.7
dp = 0.2
ax.df = tibble(x="1_1",x1=1,x2=1,i1=c(0,1),i2=1)
